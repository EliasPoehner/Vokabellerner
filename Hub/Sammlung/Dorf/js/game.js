    ...
    const S = {
      ...
      lager: { holz: 80, stein: 60, nahrung: 50, gold: 999999, eisen: 30, kohle: 40 },
    ...
    function calcLager() {
      let h = 80, s = 60, n = 50, e = 30, k = 40;
      const lc = getBuildingCount('lagerhaus');
      h += lc * 60; s += lc * 50; n += lc * 40; e += lc * 25; k += lc * 30;
    ...
    function updateUI(rates) {
      ...
      if (S.tick % 20 === 0 && currentTab === 'bauen') {
        document.querySelectorAll('.b-row[data-bid]').forEach(row => {
          ...
          // Update worker warning in detail
          const det = document.getElementById('det_' + id);
          if (det) {
            const warnEl = det.querySelector('.warn');
            const warnText = !meetsReqs ? '🔒 Voraussetzung fehlt' : (!hasW && meetsReqs && affordable ? '👷 Keine freien Arbeiter' : '');
