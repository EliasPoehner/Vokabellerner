    ...
    // Obstacle tracking
    window._obstacleGroups = [];
    window._obstacleRaycastTargets = [];
    window._obstacleMeshes = {};
    window._obstacleHoverRing = null;
    ...
    function addBuilding3D(id) {
      ...
      if (!pos) { notify('Kein Straßenzugang — baue erst Straßen in der Nähe.', 'warning'); return; }
      const { x, z } = pos;
      gridOccupied[gridKey(x, z)] = id;
    ...
    function commitMove(tx, tz) {
      if (!hasRoadAccess(tx, tz, moveBuildingId)) {
        notify('Kein Straßenzugang hier!', 'warning');
    ...
    canvas.addEventListener('mousemove', e => {
      ...
      const obsHoverHits = raycaster.intersectObjects(window._obstacleRaycastTargets || [], false);
    ...
    canvas.addEventListener('click', e => {
      ...
      if (roadMode) {
        ...
        if (!cell) return;
        const key = gridKey(cell.x, cell.z);
        if (gridOccupied[key]) { notify('Hier steht ein Gebäude!', 'warning'); return; }
        toggleRoad(key, cell.x, cell.z);
        return;
      }
      ...
      const obsClickHits = raycaster.intersectObjects(window._obstacleRaycastTargets || [], false);
      if (obsClickHits.length > 0 && obsClickHits[0].object.userData.isObstacle) {
        const obj = obsClickHits[0].object;
        removeObstacle(obj.userData.obstacleId, obj.userData.obstacleX, obj.userData.obstacleZ);
        hideObstacleHoverRing();
        return;
      }
    ...
    function clearObstacles() {
      (window._obstacleGroups || []).forEach(g => scene.remove(g));
      window._obstacleGroups = [];
      window._obstacleRaycastTargets = [];
      window._obstacleMeshes = {};
    }
    ...
    function renderObstacles() {
      ...
      S.obstacles.forEach(obs => {
        ...
        scene.add(group);
        window._obstacleGroups.push(group);
        window._obstacleMeshes[key] = group;
        group.traverse(c => {
          if (c.isMesh && c.userData.isObstacle) {
            window._obstacleRaycastTargets.push(c);
          }
        });
      });
    }
    ...
    function showObstacleHoverRing(x, z) {
      if (!window._obstacleHoverRing) {
        window._obstacleHoverRing = new THREE.Mesh(
        ...
        window._obstacleHoverRing.rotation.x = Math.PI / 2;
        window._obstacleHoverRing.position.y = 0.1;
        scene.add(window._obstacleHoverRing);
      }
      window._obstacleHoverRing.position.set(x * CELL, 0.1, z * CELL);
      window._obstacleHoverRing.visible = true;
    }

    function hideObstacleHoverRing() {
      if (window._obstacleHoverRing) window._obstacleHoverRing.visible = false;
    }
    ...
    function triggerObstacleRemovalAnim(x, z, type) {
      ...
      // Clean from tracking
      if (window._obstacleMeshes) delete window._obstacleMeshes[key];
      window._obstacleGroups = (window._obstacleGroups || []).filter(g => g !== group);
      window._obstacleRaycastTargets = (window._obstacleRaycastTargets || []).filter(m =>
    ...
    function updateMapStage(tier) {
      if (window._stageGroup) { scene.remove(window._stageGroup); window._stageGroup = null; }
    ...
    function buildTerrainMap() {
      ...
      if (!window.S || !window.S.buildings) return map;
    ...
    window.showBuildingInfo = function(bid, instIdx, grp) {
      ...
      window._selGrp = grp;
      window._selTimeout = setTimeout(() => { sel.style.display = 'none'; }, 7000);
    };
    ...
    function animate() {
      ...
      if (isNight !== _prevIsNight || window._forceNightUpdate) {
        window._forceNightUpdate = false;
      ...
      // Smoke particles
      for (const p of window._smokeParticles) {
