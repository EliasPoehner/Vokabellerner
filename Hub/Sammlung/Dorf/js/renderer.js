    ...
    function rebuild3D() {
      // Save current grid positions before clearing
      const savedPositions = {}; // buildingId -> [{x, z}, ...]
      for (const [id, groups] of Object.entries(buildingMeshes)) {
        savedPositions[id] = groups.map(g => ({ x: g.userData.gridX, z: g.userData.gridZ }));
      }
      ...
      // Re-place all buildings, reusing saved positions where available
      for (const [id, cnt] of Object.entries(S.buildings)) {
        ...
        for (let i = 0; i < cnt; i++) {
          let pos;
          if (savedPositions[id] && savedPositions[id][i]) {
            const saved = savedPositions[id][i];
            const key = gridKey(saved.x, saved.z);
            if (!gridOccupied[key]) {
              pos = saved;
            }
          }
          if (!pos) pos = findFreeCell();
          const { x, z } = pos;
